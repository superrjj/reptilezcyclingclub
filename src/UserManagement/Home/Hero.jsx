...
const Hero = forwardRef(({ refreshFunctionsRef }, ref) => {
  ...
  // Full-bleed wrapper — same negative margin trick as AboutUsSection's hero image
  const fullBleedClasses =
    'relative w-screen left-1/2 -translate-x-1/2 overflow-hidden -mt-24 mb-8 sm:mb-12';
  ...
  return (
    <div
      className={`${fullBleedClasses} group transition-all duration-700 ${
      ...
      */}
      <div className="relative w-full min-h-[55vh] sm:min-h-[60vh] md:min-h-[70vh] lg:min-h-screen aspect-[4/3] sm:aspect-[16/9] md:aspect-[21/9] lg:aspect-auto">
        {heroImages.map((image, index) => (
          <div
            key={`${image}-${index}`}
            className={`absolute inset-0 w-full h-full bg-no-repeat transition-opacity duration-700 ${
              index === currentIndex ? 'opacity-100' : 'opacity-0'
            }`}
            style={{
              backgroundImage: `url("${image}")`,
              // center-top keeps the subject (rider) visible on all sizes
